...
struct TestOutView: View {
        ...
        VStack(spacing: 12) {
            ...
            Button("Check For Teacher") {
                Task {
                    ...
                    let resposnsex: AuthenticateReturnObjct = try await ApiManager.shared.getData(from: .authenticateTeacher(company: "2001128", username: "**appi4Teacher-NoModificationxx**0", password: "123456"))
                    dump(resposnsex)
                    print("break")
    ...
    func createTeacherUser(for locationId: Int) async throws {
        ...
        do {
            ...
            print("just created teacher user for location \(locationId) and the id: \(resposnseaddAUser.id)")
    ...
    fileprivate func makeDictofTeacherUser() async throws {
        do {
            ...
            let teacherUserGroup = filteredTeacherUsers.reduce(into: [Int: Int]()) { (dict, usr) in
                dict[usr.locationId] = usr.id
            }
            
                // Output the dictionary
            for (location, id) in teacherUserGroup {
    ...
    fileprivate func makeDictofTeacherGroup() async throws {
        do {
            ...
            let TeacherGroup = filteredTeacherGroups.reduce(into: [Int: Int]()) { (dict, grp) in
                dict[grp.locationId] = grp.id
            }
            
                // Output the dictionary
            for (location, id) in TeacherGroup {
                print("Location: \(location), id: \(id)")
            }
